...
function ensureConfigured() {
    const missing = [
        ...
        !DB.articles && 'NOTION_DB_ARTICLES',
        !DB.comments && 'NOTION_DB_COMMENTS',
        !DB.inbox && 'NOTION_DB_INBOX',
        !DB.moderationLog && 'NOTION_DB_MODERATION_LOG',
    ].filter(Boolean);

    if (missing.length > 0) {
        throw new Error(`Notion 環境變數尚未完成：${missing.join(', ')}`);
    }
}
...
async function createModerationLog(input: {
    ...
    await createPage(DB.moderationLog, {
        log_title: titleProp(title),
...
export async function fetchForumPosts() {
    const res = await queryDatabase(DB.articles, {
        filter: {
            property: 'status',
...
export async function fetchComments(targetLineId: string) {
    const res = await queryDatabase(DB.comments, {
        filter: {
            and: [
...
export async function createComment(
    ...
    await createPage(DB.comments, {
        留言內容: titleProp(summarizeTitle('留言', content)),
...
export async function createContact(name: string, category: string, content: string, attachmentUrl?: string, email?: string) {
    await createPage(DB.inbox, {
        主旨: titleProp(summarizeTitle(category || '訊息', content)),
...
export async function fetchInboxMessages() {
    const res = await queryDatabase(DB.inbox, {
        sorts: [{ property: 'created_at', direction: 'descending' }],
        page_size: 100,
    });
