  ...
  return (
    <div className="min-h-screen bg-gray-50 pt-24">
      ...
      <main className="mx-auto max-w-5xl space-y-8 px-6 py-12">
        <div>
          <h1 className="mb-3 text-4xl font-black text-gray-900" style={serif}>
            觀庭筆記匯集排行
          </h1>
          <p className="text-lg leading-relaxed text-gray-600">
            依據文章互動與整體熱度整理目前已公開的觀庭筆記，方便快速查找近期被閱讀與討論的內容。
          </p>
        </div>

        <div className="rounded-3xl border border-gray-100 bg-white p-8 shadow-sm">
          {loading ? (
            <p className="text-lg font-bold text-gray-500">載入排行中…</p>
          ) : articles.length === 0 ? (
            <p className="text-lg leading-relaxed text-gray-600">目前尚無可供排序的觀庭筆記。</p>
          ) : (
            <div className="space-y-4">
              {articles.map((article, index) => (
                <Link key={article.id} href={`/forum/${article.id}`} prefetch={false} className="block">
                  <div className="rounded-2xl border border-gray-100 p-5 transition-all hover:border-[#6B8E23]/30 hover:shadow-sm">
                    ...
                    <h2 className="mb-2 text-2xl font-black text-gray-900" style={serif}>
                      {article.title}
                    </h2>
                    <div className="flex flex-wrap items-center gap-4 text-sm font-medium text-gray-500">
                      <span>{article.author || '匿名作者'}</span>
                      {article.targetSessionId ? <span>{getSessionDisplayTitle(article.targetSessionId)}</span> : null}
                    </div>
                  </div>
                </Link>
              ))}
