...
export default function SessionsHistoryV2Page() {
        ...
        <FadeIn>
          <div className="mb-8">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {(publishedSessions.length === 0 ? [{ id: 'fallback', sessionId: 's-114-1-6', title: '第十次開庭：最終言詞辯論', date: '2 月 26 日', summary: '檢察官論告與辯護律師簡報與陳述還原' }] : publishedSessions).map((ps) => (
                <Link key={ps.id} href={`/sessions/${ps.sessionId}`} className="block">
                  <motion.div whileHover={{ y: -2, scale: 1.01 }} className="group relative h-full overflow-hidden rounded-[2rem] border-2 border-orange-200 bg-white p-6 shadow-sm transition-all hover:border-orange-400 hover:shadow-xl">
                    ...
                    <div className="relative z-10">
                      ...
                      <p className="text-sm font-bold text-gray-600">{ps.summary || '包含證人詰問與重要言詞辯論實況'}</p>
                    </div>
                  </motion.div>
                </Link>
              ))}
